...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 30 13:13:46 2023
...
#number = st.sidebar.number_input("Enter Number",1.0,25.0)
                ...
                if 'weightgroup' in list(df_target.columns):
                    ...
                    if any(x.startswith('feature') for x in list(df.columns)):
                        ...
                        if len([s for s in list(df.columns) if s.startswith('feature')]) >= 2:
                            ...
                            passcode.append(1)
                            #continue to 2-2, 6
                            if sorted(list(range(1, df.filter(regex='^feature').shape[1]+1))) == sorted([int(item.split('feature')[1]) for item in list(df.filter(regex='^feature').columns)]):
                                            ...
                                            if [item for sublist in feature_cloumns_in_df_target for item in sublist] == list(df_target.filter(regex='^feature').columns):
                                                ...
                                                if str(df['weightgroup'].dtype) == 'int64':
                                                    ...
                                                    if str(df_target['weightgroup'].dtype) == 'int64':
                                                        ...
                                                        passcode.append(1)
                                                        #continue to 3-3, 13
                                                        if list(dict.fromkeys(list(df['weightgroup']))) == list(df_target['weightgroup']):
                ...
                def ipfn_df(self, df, aggregates, dimensions, weight_col='total'):
                    ...
                    for features in dimensions:
                        ...
                        for feature in product(*feat_l):
                            ...
                            if den == 0:
                                table_update[weight_col] = np.clip(table_update[weight_col], a_max=100000, a_min=10)
